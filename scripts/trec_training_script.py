...
parser.add_argument(
    ...
    type=str,
    default="data/trained_models/trec_models/trained_trec_model.tar"
)
parser.add_argument(
    "--save_path_for_vocab", help="Mention the path for saving the model.",
    type=str,
    default="data/trained_models/trec_models/vocab_dict_trec.pkl"
)
parser.add_argument(
...
parser.add_argument(
    "--test_batch_size", help="Mention the batch size for validation data.",
    type=int,
    default=32
)
parser.add_argument(
...
BATCH_SIZE_TEST = arguments.test_batch_size
...
def prepare_data_loaders(
    ...
    trainds, testds = data.TabularDataset.splits(
    ...
    print(
        "length of train and test data are respectively {} and {}".format(
            len(trainds), len(testds)
        )
    )
    print("Loading vectors from the file....")
    vec = torchtext.vocab.Vectors(glove_file_path)
    text_field.build_vocab(trainds, testds, vectors=vec, max_size=max_vocab_size)
    ...
    print(
        "Number of Batches of train and test data are respectively {} and {}".format(
            len(traindl), len(testdl)
        )
    )
    print("Size of the vocabulary is {}".format(len(text_field.vocab.stoi)))
    return traindl, testdl, text_field, label_field

if __name__ == "__main__":
    traindl, testdl, text_field, label_field = prepare_data_loaders(
    ...
    TREC_DATASET_PARAMETERS["vocab_length"] = len(text_field.vocab.stoi)
    ...
    for epoch in range(N_EPOCHS):
        ...
        valid_loss, valid_acc = evaluate(model, testdl, criterion, device=DEVICE)
